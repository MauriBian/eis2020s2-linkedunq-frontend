              ...
              </div>
{
                this.state.jobs.map( (deck,index) => {
                  return <CardDeck key={index}>
                    {
                    deck.map ((elem, index) => {
                      return <JobCard title={elem.titulo} link={elem.enlace} guest={true} text={elem.descripcion} image={elem.urlImagen} footer={elem.fechaInicioTrabajo + ' - ' + elem.fechaFinTrabajo}></JobCard>
                    })}
                  </CardDeck>
                })
              }
